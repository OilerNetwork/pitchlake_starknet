...
trait IAccount<TContractState> {
    ...
    /// @dev Warning: To guarantee the signature cannot be replayed in other accounts or other
    /// chains, the data hashed must be unique to the account and the chain.
    /// This is true today for starknet transaction signatures and for SNIP-12 signatures but might
    /// not be true for other types of signatures @param hash The hash of the data to sign
...
trait IArgentUserAccount<TContractState> {
    ...
    /// @dev Must be called by the account and authorized by the owner and a guardian (if guardian
    /// is set)
    /// @param new_security_period new delay in seconds before the escape can be completed. Must be
    /// >= 10 minutes
    ...
    /// @dev Must be called by the account and authorized by the owner and a guardian (if guardian
    /// is set)
    /// @param new_guardian_backup The address of the new backup guardian, or 0 to disable the
    /// backup guardian
    ...
    /// @dev Must be called by the account and authorized by just a guardian
    /// @dev This function assumes that there is a guardian, and that the there is an escape for the
    /// owner This must be guaranteed before calling this method, usually when validating the
    /// transaction
    ...
    /// @dev This function assumes that there is a guardian, and that the there is an escape for the
    /// guardian @dev This must be guaranteed before calling this method. Usually when validating
    /// the transaction
    ...
    /// @notice Returns the public key if the requested role is Starknet, Eip191 or Secp256k1 and
    /// panic for other types
    ...
    /// @notice Returns `0` if there's no guardian backup, the public key if the requested role is
    /// Starknet, Eip191 or Secp256k1 and panic for other types
