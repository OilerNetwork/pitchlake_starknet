...
trait IUpgradeable<TContractState> {
    /// @notice Upgrades the implementation of the account by doing a library call to
    /// `perform_upgrade` on the new implementation @param new_implementation The class hash of the
    /// new implementation @param data Data to be passed in `perform_upgrade` in the `data` argument
...
trait IUpgradableCallbackOld<TContractState> {
    /// Called after upgrading when coming from old accounts (argent account < 0.4.0 and multisig <
    /// 0.2.0)
...
trait IUpgradableCallback<TContractState> {
    ...
    /// This function is responsible for performing the actual class replacement and emitting the
