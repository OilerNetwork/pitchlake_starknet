...
async function main(environment, port) {
    ...
    const eth = new EthFacade(new Contract(ethAbi, contractAddresses.ethAddress, provider).typedv2(ethAbi));
    await eth.supplyEth(devAccount, provider, contractAddresses.ethAddress, contractAddresses.vaultAddress);
    //Can write to a file here and replace smoke test call to use multiple
    await smokeTesting(provider, contractAddresses.vaultAddress, contractAddresses.ethAddress);
}
