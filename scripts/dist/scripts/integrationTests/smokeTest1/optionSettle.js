...
export const smokeTest = async ({ provider, vaultFacade, constants: { depositAmount }, ethFacade, getLPUnlockedBalanceAll, settleOptionRoundBystander, getLiquidityProviderAccounts, getOptionBidderAccounts, }) => {
    ...
    const state = await optionRoundFacade.optionRoundContract.get_state();
    assert(state.activeVariant() === "Running", `Expected:Running\nReceived:${state.activeVariant()}`);
    const totalPremiums = await optionRoundFacade.getTotalPremiums();
    const ethBalanceBefore = await ethFacade.getBalance(liquidityProviderAccounts[0].address);
    await vaultFacade.withdraw({
        account: liquidityProviderAccounts[0],
        amount: BigInt(totalPremiums) / BigInt(4),
    });
    const ethBalanceAfter = await ethFacade.getBalance(liquidityProviderAccounts[0].address);
    const lpUnlockedBalanceAfter = await vaultFacade.getLPLockedBalance(liquidityProviderAccounts[0].address);
    ...
    assert(stateAfter.activeVariant() === "Settled", `Expected:Running\nReceived:${stateAfter.activeVariant()}`);
    checkpoint2({
        lpUnlockedBalances,
        totalPremiums,
        totalPayout,
...
async function checkpoint1({ ethBalanceBefore, ethBalanceAfter, lpUnlockedBalanceAfter, totalUnlocked, totalPremiums, }) {
    ...
    assert(BigInt(ethBalanceAfter) - BigInt(ethBalanceBefore) ===
        BigInt(totalPremiums) / BigInt(4), "LP Unlocked for A mismatch");
    assert(BigInt(lpUnlockedBalanceAfter) === BigInt(totalPremiums) / BigInt(4), "LP Unlocked for B mismatch");
    assert(BigInt(totalUnlocked) === (BigInt(3) * BigInt(totalPremiums)) / BigInt(4), "LP Locked for A mismatch");
}
async function checkpoint2({ lpUnlockedBalances, totalPremiums, totalPayout, totalLocked, totalUnlocked, depositAmount, }) {
    // - A’s unlocked balance should be 1/2 deposit amount + 1/4 total premiums - 1/2 total payout
    assert(BigInt(lpUnlockedBalances[0]) ===
        BigInt(depositAmount) / BigInt(2) +
            BigInt(totalPremiums) / BigInt(4) -
            BigInt(totalPayout) / BigInt(2), "lpUnlocked for A Mismatch");
    // - B’s unlocked balance should be 1/2 deposit amount + 1/2 total premiums - 1/2 total payout
    assert(lpUnlockedBalances[1] ===
        BigInt(depositAmount) / BigInt(2) +
            BigInt(totalPremiums) / BigInt(2) -
            BigInt(totalPayout) / BigInt(2), "lpUnlocked for B Mismatch");
    // - The total locked should == 0
    assert(Number(totalLocked) === 0, `total Locked should be zero, found: ${totalLocked}`);
    // - The total unlocked should == deposit amount + 3/4 total premiums - total payout
    assert(BigInt(totalUnlocked) ==
        BigInt(depositAmount) +
            (BigInt(3) * BigInt(totalPremiums)) / BigInt(4) -
            BigInt(totalPayout), `totalUnlocked mismatch \n${totalUnlocked}\n${lpUnlockedBalances}`);
}
