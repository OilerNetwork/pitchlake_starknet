...
export const smokeTest = async ({ provider, vaultFacade, constants, getLPLockedBalanceAll, getLPUnlockedBalanceAll, endAuctionBystander, getLiquidityProviderAccounts, }) => {
    ...
    assert(state.activeVariant() === "Auctioning", `Expected:Auctioning\nReceived:${state.activeVariant()}`);
    ...
    const lpLockedBalances = await getLPLockedBalanceAll(liquidityProviderAccounts);
    ...
    assert(stateAfter.activeVariant() === "Running", `Expected:Running\nReceived:${stateAfter.activeVariant()}`);
...
async function checkpoint1({ lpLockedBalances, lpUnlockedBalances, totalPremiums, totalLocked, totalUnlocked, constants, }) {
