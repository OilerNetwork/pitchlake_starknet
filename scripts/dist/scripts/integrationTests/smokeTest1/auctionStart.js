...
export const smokeTest = async (provider, vaultFacade, ethFacade, constants) => {
    ...
    const unlockedBalances = await vaultFacade.getLPUnlockedBalanceAll(liquidityProviderAccounts);
    const lockedBalances = await vaultFacade.getLPLockedBalanceAll(liquidityProviderAccounts);
    ...
    const ethBalancesBefore = await ethFacade.getBalancesAll(optionBidderAccounts);
    const placeBidsData = [
        {
...
async function checkpoint1({ lockedBalances, unlockedBalances, totalLockedAmount, totalUnlockedAmount, constants, }) {
    ...
    assert(Number(lockedBalances[0]) === constants.depositAmount / 2, `LockedBalanceA ${constants.depositAmount / 2} expected, found ${lockedBalances[0]}`);
    assert(Number(lockedBalances[1]) === constants.depositAmount / 2, `LockedBalanceB ${constants.depositAmount / 2} expected, found ${lockedBalances[1]}`);
    assert(Number(totalUnlockedAmount) === 0, `Total unlocked 0 expected, found ${totalUnlockedAmount}`);
    assert(Number(totalLockedAmount) === constants.depositAmount, `Total Locked amount ${constants.depositAmount} expected, found ${totalLockedAmount}`);
}
async function checkpoint2({ ethBalancesBefore, ethBalancesAfter, bidArrays, totalOptionAvailable, reservePrice, }) {
    ...
    assert(bidArrays[1][1].amount === BigInt(totalOptionAvailable) / BigInt(2), "First bid for B amount wrong");
    assert(bidArrays[1][1].price === BigInt(2) * BigInt(reservePrice), "First bid for B price wrong");
    assert(bidArrays[1][0].amount === BigInt(totalOptionAvailable) / BigInt(2), "Second bid for B amount wrong ");
    assert(BigInt(bidArrays[1][0].price) === BigInt(reservePrice), `Second bid for B price wrong.\n Expected:${reservePrice}, Actual:${bidArrays[1][0].price}`);
}
