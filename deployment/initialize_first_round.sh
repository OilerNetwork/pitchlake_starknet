...
for ((i = 1; i <= $VAULT_COUNT; i++)); do
	...
	MAX_RETURNS_CALCULATION_WINDOW_SECONDS=$((ROUND_DURATION * 3))
	...
	echo "Max returns calculation window: $MAX_RETURNS_CALCULATION_WINDOW_SECONDS seconds"
	...
	MAX_RETURNS_FROM=$(($TIMESTAMP - $MAX_RETURNS_CALCULATION_WINDOW_SECONDS))
	...
	echo "Max returns from: $(date -r $MAX_RETURNS_FROM)"
	...
	FOSSIL_RESPONSE=$(curl -X POST "$FOSSIL_API_URL/pricing_data" \
		...
		-d "{
            ...
            \"params\": {
                ...
                \"max_returns\": [$MAX_RETURNS_FROM, $TIMESTAMP],
