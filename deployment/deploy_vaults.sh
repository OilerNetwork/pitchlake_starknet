...
for ((i=0; i<$VAULT_COUNT; i++)); do
    ...
    # Deploy Vault
    ADDRESS=$(starkli deploy $VAULT_HASH $FOSSIL_CLIENT_ADDRESS $ETH_ADDRESS $OPTIONROUND_HASH $ALPHA $STRIKE $ROUND_TRANSITION_DURATION $AUCTION_DURATION $ROUND_DURATION --watch | grep -o '0x[a-fA-F0-9]\{64\}' | head -1)
