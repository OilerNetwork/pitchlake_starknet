use pitch_lake_starknet::{tests::{utils::lib::{variables::{decimals, week_duration}}}};

#[derive(Copy, Drop, Serde, starknet::Store, PartialEq)]
struct OptionRoundParams {
    // @note Discuss if we should set these when previous round settles, or shortly after when this
    ...
    // that way if the round's auction start is delayed (due to collateral requirements), we can set
    // a proper auction end time when it eventually starts ?
...
fn mock_option_params() -> OptionRoundParams {
    ...
    // minimum_collateral_required:u256 // the option round will not start until this much
