...
fn test_round_start_auction_success() {
    ...
    let success  : bool = vault_dispatcher.start_auction(mock_option_params());
    assert(success == true, 'should be able to start');
    assert_event_auction_start(vault_dispatcher.get_option_round_params().total_options_available);
}
...
fn test_round_state_auction_ended() {
    ...
    assert_event_auction_settle(vault_dispatcher.get_auction_clearing_price());
...
fn test_claim_payout_failure() {
    ...
    set_contract_address(option_bidder_buyer_1());
    vault_dispatcher.auction_place_bid(bid_amount_user_1, bid_price_user_1 );
    vault_dispatcher.claim_payout(option_bidder_buyer_1());   // should fail as option has not settled
...
fn test_transfer_to_vault_failure() {
    ...
    vault_dispatcher.settle_auction();
    set_contract_address(liquidity_provider_1());
    vault_dispatcher.transfer_collateral_to_vault(liquidity_provider_1());   // should fail as option has not settled
