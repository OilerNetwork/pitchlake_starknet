...
//  @note If we allow this, the premiums still need to be collected, since they are already sitting
//  in the next round, we can mark the current round's premiums collected, and leave the amount
//  sitting in the next round
#[ignore]
#[test]
#[available_gas(50000000)]
...
// @dev Check unallocated assertions after speaking with Dhruv, should unallocated_balance be
// premiums + next_round_deposit, or just next_round_deposit ?
// @dev Is this test suffice for knowing withdrawCheckpoint and current_roundPosition is updated
// correctly? If lp_collateral is correct then withdrawCheckpoint must be right ?
#[ignore]
#[test]
#[available_gas(50000000)]
fn test_convert_position_to_lp_tokens_success() { //
    ...
    // @dev Some of LP1's collateral is now represented as tokens, this means their collateral will
    // decrease, but the round's will remain the same.
    assert(
...
//    assert(next_round_unallocated2 == deposit_amount_wei / 2, 'all liq shd be unalloc in
//    next');

    // Assert converting tokens -> position while current is Auctioning deposits into the next next
// round
...
// Test converting round lp tokens into a position backwards fails (only if user can choose target
// round)
...
// Assert LP3's r3 position is < LP2's r3 position (LP3's position does not include the premiums
// from r1 rolling over, but LP2's does)
// assert(vault::position[LP3, 3] < vault::position[LP2, 3])
}


// Test converting lp tokens into a position in same round (r1 tokens to r1 position) sets
// premiumsCollected to true
//  - The minter of the rx tokens already collected their premiums, this ensures that if the buyer
//  of the tokens converts the rx tokens into an rx position, they are not allowed to double-collect
//  these premiums
...
// vault.convert_lp_tokens_to_position(lp_token_id: 1, target_round_for_position: 1, amount: 5 ETH
// LP tokens, caller: LP1)
// vault.convert_lp_tokens_to_position(lp_token_id: 1, target_round_for_position: 1, amount: 5 ETH
// LP tokens, caller: LP2)
...
// @note Both LP's should not have any premiums to claim, when LP1 converted position -> token, they
// already collected the premiums for the tokens assert(vault.premiums_balance_of(LP1) == 0)
...
// Test converting lp tokens into a position in the same round (r1 tokens to r1 position) handles
// exisiting premiums @dev When rX tokens -> rX position, we set premiumsCollected to true. If the
// user has an active position with collectable premiums, we do not want them to get lost, so we
// collect them during this step.
...
// - @dev LP tokens represent a position net premiums, so we do not count the premiums earned from
// an lp token's source round.
// This is an issue because if a user converts tokenAs -> tokenBs, in the future we will ingore the
// premiums earned from roundB. To make sure the user still gets their premiums from this round upon
// tokenA->tokenB conversion, we need to collect the premiums from roundB for the user (we collect
// the amount of premiums the tokens earn in rB, if the user already has a storage position in rB
// with collectable premiums we ignore them).
// - @dev When we collect rB's premiums, we do not touch premiumsCollected or collectable premiums
// in rB, they can stay as they are in storage - @dev When we collect rB's premiums, we deposit them
// into roundB+1 as a position.
//    - We cannot collect the premiums as ETH since rB might be a historical (not the current) round
//    and that ETH may be locked in the current round. If B is the current round, then this position
//    in rB+1 is immediately withdrawable.

// @dev Begs the question, should collect always be a deposit into the next round, to then be
// withdrawable if choosen to (if user wants to claim premiums as eth, can just do a multi-call,
// collect, then withdraw)
//  - `collect()` should not always set premiumsCollected to true. If a user converts tokens A-B, we
//  collect the rB premiums for the tokens. If the user has a position in storage with collectable
//  premiums, we are not touching them, so premiumsCollected does not need to be set. So maybe the
//  collect entry point does set premiums collected to true, but collecting during a tokenA-tokenB
//  conversion does not
