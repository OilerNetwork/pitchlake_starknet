...
fn test_clearing_price_1() {
    ...
    let bid_amount: u256 = bid_count
        ...
        * vault_dispatcher.decimals().into();
...
fn test_bid_below_reserve_price() {
    ...
    let success = vault_dispatcher
        .auction_place_bid(10 * vault_dispatcher.decimals().into(), bid_below_reserve);
...
fn test_option_balance_per_bidder_after_auction_3() {
    ...
    assert(options_created_user_6_count == 0, 'options shd match');
    assert(
        unused_bid_amount_user_3 == 27 * vault_dispatcher.decimals().into(),
