...
fn test_withdraw_from_deposits_eth_transfer() {
    ...
    // Deposit liquidity (goes to next round)
...
// @note add assertion that vault::round_positions[lp, next_id] updates to dynamic value (need to add vault entry point for get_lp_deposit_in_round)
//  - withdrawing while current is settled requires us to update the checkpoint (and position to dynamic value - withdraw amount)
#[test]
#[available_gas(10000000)]
