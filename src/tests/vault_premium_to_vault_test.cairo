...
fn test_paid_premium_withdrawal_to_liquidity_provider() {
    ...
    let expected_unallocated_wei: u256 = vault_dispatcher
        .get_auction_clearing_price(option_round_id)
        * vault_dispatcher.total_options_sold();
...
fn test_paid_premium_withdrawal_to_invalid_provider() {
    ...
    let success: bool = vault_dispatcher.withdraw_liquidity(lp_id, expected_unallocated_wei);
    assert(
        success == false, 'should not be able withdraw'
    ); // invalid liquidity provider withdrawal
}
...
fn test_premium_collection_ratio_conversion_unallocated_pool_1() {
    ...
    let total_premium_to_be_paid: u256 = vault_dispatcher
        .get_auction_clearing_price(option_round_id)
        * vault_dispatcher.total_options_sold();
