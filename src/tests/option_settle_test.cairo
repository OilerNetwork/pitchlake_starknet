...
fn test_invalid_user_collection_of_payout_after_settle() {
    ...
    let claimed_payout_amount :u256= vault_dispatcher.claim_option_payout(option_round_id, option_bidder_buyer_2()); 
...
fn test_collection_of_premium_after_settle() {
    ...
    let claimable_premium_amount: u256 = vault_dispatcher.premium_balance_of(lp_id); // this will collect the premium back into unallocated_pool in the vault   
    let unallocated_wei_after_premium:u256 = vault_dispatcher.total_unallocated_liquidity();

    assert(claimable_premium_amount == vault_dispatcher.total_options_sold() * vault_dispatcher.get_auction_clearing_price(option_round_id) , 'premium amount shd match');
...
fn test_option_payout_1() {
    ...
    let payout_balance_expected = vault_dispatcher.option_balance_of(option_bidder_buyer_1()) * (settlement_price - option_params.strike_price); // TODO convert this to gwei instead of wei
...
fn test_option_post_payout_collaterized_count_1() {
    ...
    set_contract_address(option_bidder_buyer_1());
    let claimed_payout :u256 = vault_dispatcher.claim_option_payout(option_round_id, option_bidder_buyer_1());

    let total_collaterized_count_after_payout_claimed : u256= vault_dispatcher.total_collateral();
...
fn test_option_post_payout_collaterized_count_3() {
    ...
    let total_collaterized_count_after_claim : u256= vault_dispatcher.total_collateral();
